          ...
          <div class="box box-primary">
            ...
            <div class="box-body">
              <table id="list-table" class="table table-bordered table-striped">
                ...
                <tbody>
                @foreach($seattypes as $indx => $seattype)
                <tr>
                    <td>{{ $indx + 1 }}</td>
                    <td>{{ $seattype->name }}</td>
                    <td>{!! getStatusLabel($seattype->status) !!}</td>
                    <td>
                        <button class="btn bg-navy" title="" data-original-title="Edit Seat Type" onclick="openEditModal('{{ route('admin.seat-types.update', $seattype->id) }}', '{{ $seattype->name }}','{{ $seattype->status }}')"><i class="fa fa-edit"></i> Edit</button>
                        <a href="{{ route('admin.seat-types.destroy', $seattype->id) }}" class="btn btn-danger" data-confirm-delete="true" title="" data-original-title="Delete Seat Type"><i class="fa fa-trash"></i> Delete</a>
                    </td>
                </tr>
                @endforeach
                </tbody>
